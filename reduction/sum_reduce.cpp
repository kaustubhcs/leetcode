...
int get_sending_id(int my_id, int root_id, int step_id) { // Based in
    ...
    // Next, we determine whether to send even indices data to odd indices OR send odd indices data to even indices.
    if (( (root_id / pow(2,step_id)) % 2) == EVEN) {
        // For this case the next index of root id will be even.
        ...
        // PS: Bad idea to divide int by int. Use floor function instead.
        keep_even_index = true;
    } else {
        // Else, keep data on ODD indices
        keep_odd_index = true;
    }
...
void reduce(int root, void* buf, int sz) {
    ...
    // In the end, the final sum will automagically appear in the root node.
}